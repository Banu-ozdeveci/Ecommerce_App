...
import { removeProductFromUsersBag } from "../API/index";
...
export const ShoppingProduct = connect(mapStateToProps)(
  ({ product, count, onPress, orders }) => {
    const { brandName, name, price, url, hprice, IsNew, id } = product;

    const removeShoppingProduct = async () => {
      try {
        removeProductFromUsersBag(product);
      } catch (error) {
    ...
    return (
      <Swipeable
        renderRightActions={() => (
          ...
          >
            <TouchableOpacity onPress={() => removeShoppingProduct()}>
            ...
            <View style={styles.row}>
              ...
              <View style={styles.column}>
                ...
                <View style={{ flexDirection: "row", left: 25 }}>
                  ...
                  {orders ? null : <Text>Selected Count: {count}</Text>}
                </View>
              </View>
            </View>
          </TouchableOpacity>
